...
## 🔒 セキュリティとプライバシーへの配慮
**個人情報の取り扱いについて**:
本アプリケーションは、TDEE（総エネルギー消費量）算出のためにユーザーの身体情報（身長・体重等）を取り扱います。

現在はポートフォリオとしての機能実装を優先していますが、**実サービスとして運用する際には、以下のセキュリティ対策が必須要件であると認識しており、将来的な実装ロードマップに含めています。**

1.  **ログのマスキング**: アプリケーションログへの個人情報の出力をシステムレベルで制限する。
2.  **DB暗号化**: データベース内の機微なカラム（身長、体重）を暗号化して保存する。
3.  **同意プロセスの実装**: プライバシーポリシーへの同意およびデータ処理に対する明示的な許諾フローをUIに組み込む。
4.  **データの最小化**: サービスの提供に不可欠な情報のみを収集・保持する設計を徹底する。

## 📊 データソース
本アプリケーションは、[Open Food Facts](https://world.openfoodfacts.org/) のデータを利用しています。
データは [Open Database License (ODbL)](https://opendatacommons.org/licenses/odbl/1.0/) の下で提供されています。

### APIレート制限への配慮
Open Food Facts のAPI利用ポリシーを尊重するため、本アプリケーションでは検索結果に対してサーバーサイドキャッシュ（TTL: 5分）を実装しています。
これにより、外部サーバーへの負荷を軽減し、頻繁に検索されるキーワードに対するレスポンス速度を向上させています。
